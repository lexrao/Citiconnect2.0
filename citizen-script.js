...
function saveReportToDB(report) {
    ...
    // Remove undefined fields and skip large photo blobs from Firestore
    // (store photo metadata only to stay within Firestore 1MB doc limit)
    if (reportToSave.photos && reportToSave.photos.length > 0) {
        reportToSave.photos = reportToSave.photos.map(p => ({ name: p.name }));
    }
    ...
    return db.collection('reports').doc(report.id).set(reportToSave)
        .then(() => console.log('Report saved to Firestore'));
}
...
async function trackMyReport() {
    ...
    resultEl.innerHTML = `<div style="text-align:center;padding:30px;color:#6b7280;">
        <div style="font-size:2em;margin-bottom:8px;">🔄</div>
        <div style="font-weight:600;">Searching for report…</div>
    </div>`;
    ...
    if (!report) {
        resultEl.innerHTML = `
        <div style="padding:20px 24px;background:#fef2f2;border-radius:12px;border-left:4px solid #ef4444;display:flex;align-items:flex-start;gap:14px;">
            <span style="font-size:1.8em;line-height:1;">❌</span>
            <div>
                <div style="font-weight:700;color:#991b1b;font-size:1em;margin-bottom:4px;">Report Not Found</div>
                <div style="color:#b91c1c;font-size:0.88em;">No report found with ID <strong>${rawId}</strong>. Please double-check your reference ID and try again.</div>
                <div style="color:#b91c1c;font-size:0.82em;margin-top:6px;">💡 Your reference ID was shown after submission (e.g. CR-2024-001)</div>
            </div>
        </div>`;
        return;
    }
    ...
    const statusMeta = {
        'Pending':     { color: '#f59e0b', bg: '#fef3c7', barBg: '#fef9e7', icon: '🕐', label: 'Pending Review',    desc: 'Your report has been received and is in the queue. Our team will review it shortly.' },
    ...
    const timelineHTML = steps.map((step, i) => {
        ...
        <div style="display:flex;flex-direction:column;align-items:center;flex:1;position:relative;">
            ${i > 0 ? `<div style="position:absolute;top:18px;right:50%;width:100%;height:3px;background:${lineColor};z-index:0;"></div>` : ''}
            <div style="width:38px;height:38px;border-radius:50%;background:${done ? meta.color : '#e5e7eb'};display:flex;align-items:center;justify-content:center;font-size:1.1em;z-index:1;position:relative;box-shadow:${active ? '0 0 0 4px ' + meta.color + '33' : 'none'};transition:all 0.3s;">
                ${done ? step.icon : '<span style="color:#9ca3af;font-size:0.85em;">○</span>'}
            </div>
            <div style="margin-top:8px;font-size:12px;font-weight:${active ? '700' : '500'};color:${done ? meta.color : '#9ca3af'};text-align:center;">${step.label}</div>
        </div>`;
    }).join('');
    ...
    resultEl.innerHTML = `
    <div style="border:2px solid ${meta.color};border-radius:16px;overflow:hidden;animation:fadeIn 0.3s;">
        <!-- Status Header -->
        <div style="background:${meta.bg};padding:20px 24px;display:flex;align-items:flex-start;gap:16px;border-bottom:1px solid ${meta.color}30;">
            <div style="font-size:2.4em;line-height:1;">${meta.icon}</div>
            <div style="flex:1;">
                <div style="font-weight:800;color:${meta.color};font-size:1.15em;">${meta.label}</div>
                <div style="color:#374151;font-size:0.88em;margin-top:4px;">${meta.desc}</div>
            </div>
            <div style="text-align:right;flex-shrink:0;">
                <div style="font-size:11px;color:#6b7280;font-weight:600;text-transform:uppercase;letter-spacing:.5px;">Reference ID</div>
                <div style="font-size:1.1em;font-weight:800;color:#1f2937;font-family:'Courier New',monospace;">${report.id}</div>
            </div>
        </div>

        <!-- Timeline Bar -->
        <div style="background:${meta.barBg};padding:22px 28px;border-bottom:1px solid #f3f4f6;">
            <div style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.6px;margin-bottom:16px;">Progress</div>
            <div style="display:flex;align-items:flex-start;gap:0;position:relative;">${timelineHTML}</div>
        </div>

        <!-- Report Details -->
        <div style="padding:22px 24px;background:white;">
            <div style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.6px;margin-bottom:14px;">Report Details</div>
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;">
                <div style="background:#f9fafb;border-radius:10px;padding:12px 16px;">
                    ...
                    <div style="font-weight:700;color:#1f2937;font-size:0.95em;">${report.category}</div>
                </div>
                <div style="background:#f9fafb;border-radius:10px;padding:12px 16px;">
                    <div style="font-size:11px;color:#9ca3af;font-weight:600;margin-bottom:3px;">LOCATION</div>
                    <div style="font-weight:700;color:#1f2937;font-size:0.95em;">📍 ${report.location}</div>
                </div>
                <div style="background:#f9fafb;border-radius:10px;padding:12px 16px;">
                    <div style="font-size:11px;color:#9ca3af;font-weight:600;margin-bottom:3px;">DATE SUBMITTED</div>
                    <div style="font-weight:700;color:#1f2937;font-size:0.95em;">📅 ${report.date.toLocaleDateString('en-PH', { year:'numeric',month:'short',day:'numeric' })}</div>
                </div>
                <div style="background:#f9fafb;border-radius:10px;padding:12px 16px;">
                    <div style="font-size:11px;color:#9ca3af;font-weight:600;margin-bottom:3px;">PRIORITY</div>
                    <div style="font-weight:700;color:${priorityColor};font-size:0.95em;">⚡ ${report.priority}</div>
                </div>
                ${report.responseTime != null ? `
                <div style="background:#f0fdf4;border-radius:10px;padding:12px 16px;border:1px solid #d1fae5;">
                    <div style="font-size:11px;color:#059669;font-weight:600;margin-bottom:3px;">RESPONSE TIME</div>
                    <div style="font-weight:700;color:#065f46;font-size:0.95em;">⏱️ ${report.responseTime} day${report.responseTime !== 1 ? 's' : ''}</div>
                </div>` : ''}
            </div>
            ${report.description ? `
            <div style="margin-top:12px;background:#f9fafb;border-radius:10px;padding:14px 16px;">
                <div style="font-size:11px;color:#9ca3af;font-weight:600;margin-bottom:6px;">DESCRIPTION</div>
                <div style="color:#374151;font-size:0.9em;line-height:1.6;">${report.description}</div>
            </div>` : ''}
        </div>

        <!-- Footer note -->
        <div style="padding:14px 24px;background:#f9fafb;border-top:1px solid #f3f4f6;display:flex;align-items:center;gap:8px;">
            <span style="font-size:1em;">💬</span>
            <span style="font-size:0.82em;color:#6b7280;">For urgent concerns, visit or call the Barangay Hall directly. Status updates are sent via SMS and email when available.</span>
        </div>
    </div>`;
}
