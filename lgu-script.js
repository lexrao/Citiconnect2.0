...
function viewReportDetails(reportId) {
    ...
    // Admin work proof display (if any uploaded)
    const workProofHTML = (report.workProof && report.workProof.length > 0) ? `
    <div style="margin-top:20px;border-top:2px solid #10b981;padding-top:20px;">
        <div style="font-size:12px;font-weight:700;color:#065f46;text-transform:uppercase;letter-spacing:.6px;margin-bottom:10px;">🛠️ Completion Proof (${report.workProof.length})</div>
        <div style="display:flex;flex-wrap:wrap;gap:10px;">
            ${report.workProof.map((p, i) => `
                <div style="position:relative;width:110px;height:110px;border-radius:10px;overflow:hidden;border:2px solid #10b981;cursor:pointer;flex-shrink:0;"
                    onclick="lguOpenPhotoLightbox('${p.url.replace(/'/g, "\\'")}','${(p.name||'Proof '+(i+1)).replace(/'/g,"\\'")}')">
                    ${p.url.match(/\.(mp4|webm|mov)$/i) ? 
                        `<video src="${p.url}" style="width:100%;height:100%;object-fit:cover;"></video><div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:32px;height:32px;background:rgba(16,185,129,0.9);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:16px;pointer-events:none;color:white;">▶</div>` :
                    ...
                    <div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.5);color:white;font-size:8px;padding:2px 5px;text-overflow:ellipsis;overflow:hidden;white-space:nowrap;">✓ Uploaded</div>
                </div>`).join('')}
        </div>
    </div>` : '';
